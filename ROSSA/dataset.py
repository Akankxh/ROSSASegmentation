...
class OCTADataset(data.Dataset):
    def __init__(self, train=True, subfolder = 'train/image', lblenc = None, maskfolder = 'train/label', augmen = None):
        ...
        if self.train:
            self.data = glob.glob(os.path.join('ROSSA','dataset','train','image','*.png'))
        else:
            self.data = glob.glob(os.path.join('ROSSA', 'dataset','val','image','*.png')) 
...
if __name__ == '__main__':
    ...
    encoder=LabelGeneration(subfolder=maskfolder) 
    dataset = OCTADataset( lblenc=encoder, subfolder=subfolder ) 
    print(len(dataset))
    # print(len(dataset))
