...
class RecurrentConvNeXtBlock(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6):
        ...
        self.norm = nn.GroupNorm(num_groups=dim//16, num_channels=dim, eps=1e-6)
        self.pwconv1 = RecurrentBlock(dim, k_size=3)
        self.act = nn.GELU()
        self.pwconv2 = RecurrentBlock(dim, k_size=3)
        
        # NEW: Large Kernel Attention for Global Context
        self.lka = LKA(dim)

        # NEW: Squeeze-and-Excitation Block for Channel Attention
        self.se = SEBlock(dim)

        # NEW: Dilated Residual Block for Multi-scale Feature Extraction
        self.residual = ResidualBlock(dim, dim, dilation=2)
    ...
    def forward(self, x):
      ...
      x = self.norm(x)  # Apply GroupNorm in (N, C, H, W) format

      x = self.pwconv1(x)
      x = self.act(x)
      x = self.pwconv2(x)

    # Apply Large Kernel Attention (LKA)
      x = self.lka(x)

    # Apply SE Channel Attention
      x = self.se(x)

    # Apply Dilated Residual Block for Multi-scale Context
      x = self.residual(x)

      if self.gamma is not None:
        x = self.gamma.view(-1, 1, 1) * x  # Reshape gamma to (C, 1, 1)
