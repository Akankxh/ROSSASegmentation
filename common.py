...
class RecurrentBlock(nn.Module):
    """
    Recurrent Block with R iterations for feature refinement.
    """
    def __init__(self, out_ch, k_size, t=2, groups=1):  # R=3 recurrence
...
class RecurrentConvNeXtBlock(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6):
        super().__init__()
        self.dwconv = RecurrentBlock(dim, k_size=9, groups=dim)  # 9x9 depthwise conv
        self.norm = nn.GroupNorm(num_groups=dim//16, num_channels=dim, eps=1e-6)  # GroupNorm instead of LayerNorm
        self.pwconv1 = RecurrentBlock(dim, k_size=3)
        #self.swin_attn = SwinTransformerBlock(dim=dim, num_heads=4)  # Adding Swin Attention
        self.act = nn.GELU()
    ...
    def forward(self, x):
        input = x
        x = self.dwconv(x)
        x = self.norm(x)  # Apply GroupNorm
        x = self.pwconv1(x)
        #x = self.swin_attn(x)  # Apply Swin Transformer Attention
        x = self.act(x)
        x = self.pwconv2(x)
